...
def get_clustering_data():
    ...
    # matched_sine = make_matched_boundaries(n_samples=1000, 
    #                                      boundary_width=0.2, 
    #                                      boundary_type='sine', 
    #                                      random_state=42)
    ...
    # diffusion = make_diffusion_circular(n_points=1000, layers = 8, inter_radius=1.0, outer_radius=2.0 ,init_density = 300, decay=0.5)

    blob_size_dis = make_blobs_size_dis(size_ratio=1.0, center_dis=6.5, n_samples=1500)
...
def remove_top_k_edges(mst: list, k: int) -> list:
    """
    Remove k largest weighted edges from MST.
    
    Parameters:
    -----------
    mst : list of tuples (v1, v2, weight)
    ...
    Returns:
    --------
    list of tuples (v1, v2, weight)
        MST with k largest weighted edges removed
    """
    # Sort edges by weight in descending order
    sorted_edges = sorted(mst, key=lambda x: x[2], reverse=True)
    
    # Return all edges except the k largest ones
...
def main():
    ...
    plot_datasets(datasets, titles=['Straight Boundary', 'Sine Boundary'])
    for dataset in datasets:
        ...
        # the final clusters after cutting
        graph_after_cut = remove_top_k_edges(mst, k=1)
        plotter.plot_mst_2d(graph_after_cut, intermediate=False, plot_cluster=False, num_clusters=num_clusters[cnt], removed_edges=True)
