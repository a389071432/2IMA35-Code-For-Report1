...
def get_clustering_data(r2, seed):


    circles = final_make_multiple_circles([
                                     {'c': [0,0], 'r': 1.0, 'n': 800},
                                     {'c': [r2+1.0+0.1, 0], 'r': r2, 'n': 800}
                                    ], seed)
...
def main():
   ...
   seeds = [
       1057142517, 1547905310, 150638261, 1184015999, 1220305740,
       592933569, 1835325023, 1366467766, 796748167, 267472915,
       720055956, 33950755, 662283339, 155879597, 719437072,
       229956126, 1333240072, 954253822, 501530068, 1971358834,
       1545259330, 1681777861, 2130101816, 513515885, 1214694088,
       1661238396, 1471527061, 1623579914, 1377061228, 1080205204,
       171802580, 2030230608, 136947588, 1914537629, 1911297726,
       722527325, 629919183, 947339285, 772027100, 336046122,
       1178755653, 350040802, 113372883, 258840583, 855652072,
       691027101, 481897223, 1857373369, 1422369516, 1304462184
   ]
   ...
   for seed in seeds:
       ...
       runs_list, graph_list = [], []
       yhats_list, contracted_leader_list = [], []
       msts = []

       # Binary search loop
       while RANGE >= 0.02 and L <= R:
           r2 = mid
           print(f'checking r2={r2}')
           
           # Generate and process dataset
           dataset = get_clustering_data(r2, seed)
           V = [item for item in dataset[0][0]]
           
           # Run clustering algorithm
           runs, graph, yhats, contracted_leader, mst, leaders = run(
               V, 
               8, 
               data_reader, 
               beta, 
           ...
           if len(graph) == 2 and calcu_acc(yhats[-1], dataset[0][1])>=0.997:
              L = mid
           else:
               R = mid  
           mid = (L + R) / 2
