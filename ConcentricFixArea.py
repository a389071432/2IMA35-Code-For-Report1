...
def get_clustering_data():
    ...
    # varied = make_blobs(n_samples=1500,
    #                     cluster_std=[1.0, 1.0],
    #                     )
    ...
    # matched_sine = make_matched_boundaries(n_samples=2000, 
    #                                      boundary_width=0.07, 
    #                                      boundary_type='sine', 
    #                                      random_state=42)

    # interleaved = make_matched_boundaries(n_samples=2000, 
    #                                      boundary_width=0.25, 
    #                                      boundary_type='interleaved', 
    #                                      random_state=42)

    # blur_boundary = make_blur_boundaries(n_samples=2000, 
    #                                      boundary_width=0.11,
    #                                      blur = 0.15, 
    #                                      boundary_type='sine', 
    #                                      random_state=42)

    # diffusion = make_simple_diffusion(n_points=1000, decay=2)

    # diffusion = make_diffusion_exp(layers = 8, right=0.5 ,init_density = 300, decay=1.0)

    # diffusion = make_diffusion_circular(n_points=1000, layers = 1, inter_radius=1.45, outer_radius=2.0, init_density = 750, decay=0.3)

    # moon_noised = make_moon_noised(n_samples=10000, noise=0.05, num_noise_points=250, noise_bounds=None)

    # circle_noised = make_circle_noised(n_samples=500, noise=0.05, num_noise_points=250, noise_bounds=None)

    # close_concentric = make_close_concentric(n_samples=1000, r=1.0, boundary_width=0.2, random_state=None, min_dist=0.099, max_dist=0.1)


    # blob_size_dis = make_blobs_size_dis(size_ratio=0.9, center_dis=0.2, var = 0.05, n_samples=1500)

    concentric = final_make_concentric(
                   {'inner': 0, 'outer': 1.5, 'n':1200},
                   {'inner': 1.65, 'outer': 2.0, 'n': 600}, 42
       )
...
def affinity_clustering(adj, vertex_coordinates, plot_intermediate, num_clusters=2, plotter=None):
    ...
    while i < imax:
        ...
        plotter.plot_cluster(yhat, None, vertex_coordinates, i, leader, 'D:/courseProjects/Massive Parallel/MSTforDenseGraphs/Results_buckets/concentric/fix_area/')
        # if len(graph)<30:
