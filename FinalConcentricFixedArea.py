...
def get_clustering_data(n2, seed):
    ...
    concentric = final_make_concentric(
                   {'inner': 0, 'outer': 1.5, 'n':1250},
                   {'inner': 1.65, 'outer': 2.0, 'n': n2}, seed=seed
       )
...
def affinity_clustering(adj, vertex_coordinates, plot_intermediate, num_clusters=2, plotter=None):
    ...
    while i < imax:
        ...
        # plotter.plot_cluster(yhat, None, vertex_coordinates, i, leader, 'D:/courseProjects/Massive Parallel/MSTforDenseGraphs/Results_buckets/two circles/fix_area/')
...
def main():
    ...
    seeds = [10650, 46645, 73815, 48229, 33349, 28835, 58822, 61243, 46959, 2381, 87254, 74712, 54397, 26302, 3922, 13052, 45970, 73118, 21931, 25373]
    for i in range(16):      
        succ = 0    
        density_ratio = 1 + 0.1 * i     
        for k in range(20):
            dataset = get_clustering_data(int(800*density_ratio), seed=seeds[k])
        ...
        print(f'density_ratio = {density_ratio}, succ rate={succ/20}')
